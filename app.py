...
# ==========================================
    ...
    st.warning("""⚠️ **バックテスト結果について**
過去日付での検証は参考値です。現在のモデルは最新データ（学習データの最終日まで）を使って学習されているため、
過去日付でのバックテストでは「その時点では知り得なかった情報」が一部混入します（特に馬のコース適性・直近成績）。
**穴馬回収率が特に高く出る場合は、この影響が大きい可能性があります。**
直近1〜2週間のバックテストが最も信頼性が高いです。""")
